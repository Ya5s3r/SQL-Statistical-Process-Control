...
GO
/****** Object:  StoredProcedure [dbo].####    Script Date: 07/02/2020 10:12:17 ******/
SET ANSI_NULLS ON
GO
...
-- Description:      Statistical Process Control SQL query - XmR Chart
-- =============================================
...
--- Enter select statement below so it aligns with format of #SPCTestData
--- Example:
/*
SELECT 
	   CAST(dateadd(month,datediff(month,0,Date),0) AS Date) as SampleDate
	  ,LEFT(ReferralOrganisation, 4) as AreaCode
	  ,ReferralOrganisationGroup as AreaName
	  ,'Ref' as MeasureCode
	  ,'Referrals' as MeasureName
	  ,COUNT(Activity) as Value
  FROM ...

  Use the @refstart variable here to indicate start year/month of activity
  Could also use the @organisation variable to specify service/organisation if needed
*/
...
Insert into #SPCTestIndicator values  ('Ref',@target,'0')
...
---MEAN OVER SPECIFIED BASE PERIOD -- this prevents the query recalculating the control limits every time the 
--query is run
---the original query didn't support this
...
INSERT into #Averagebase SELECT
                ...
                AVG(value) AS Mean
                FROM #SPCTestData
				where SampleDate BETWEEN @sdatebase AND @edatebase --specified at beginning 
                GROUP BY 	
                AreaCode,
                MeasureCode,
                MeasureName
...
ROUND(AVGB.Mean - (MovingRangeAverage * 2.66), 2) AS LCL,  ---LOWER CONTROL LIMIT
ROUND(AVGB.Mean + (MovingRangeAverage * 2.66), 2) AS UCL,  ---UPPER CONTROL LIMIT
...
LEFT JOIN --- Calculating Moving range Average OVER SPECIFIED BASE PERIOD - Adjusted from original query
                    (
                             SELECT
                             a.AreaCode,
                             a.MeasureCode,
